...
class CommandFactory {
  ...
  fun preprocessArgs(cmd: CommandType, args: Array<String>): Arguments {
    ...
    val textToInsert = args.getOrNull(6)
    ...
    val pathType = if (sourcePath.exists() && sourcePath.canWrite()) {
      if (sourcePath.isDirectory) {
        ...
        if (args[1].endsWith(".docx")) { PathType.DOCX } else { PathType.SINGLE }
      }
      else { PathType.UNKNOWN } // Hell froze over and the compiler is satisfied
    }
    else {
    ...
    return Arguments(pathType, command, sourcePath, maxSize,
                     extension, lineNumber, textToFind, textToInsert)
  }
